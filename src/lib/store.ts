...
type DeskState = {
  ...
  applyHistoryReset: (at: number) => boolean;
  ...
  confirmMessageOrigin: (origin: string) => void;
  ...
  persist(
    (set, get) => ({
      ...
      setConnection: (patch) => {
        ...
        set((s) => ({
          ...
          // Settings edits update connection.origin before validation. Keep the
          // history's prior ownership until a catalogue confirms the new one.
          messageOrigin: origin !== prevOrigin && !s.messageOrigin && s.messages.length ? prevOrigin : s.messageOrigin,
        }));
      },
      applyHistoryReset: (at) => {
        if (!Number.isSafeInteger(at) || at <= 0 || at <= get().historyResetAt) return false;
        set((s) => ({ historyResetAt: at, messages: s.messages.filter(m => m.createdAt > at), approvals: [], unreadBots: {}, activity: {}, botState: {} }));
        return true;
      },
      confirmMessageOrigin: (origin) =>
        set((s) => ({
          messages: shouldClearMessagesForOrigin(s.messageOrigin, origin) ? [] : s.messages,
          historyResetAt: shouldClearMessagesForOrigin(s.messageOrigin, origin) ? 0 : s.historyResetAt,
      ...
      syncHermesProfiles: (profiles, _capabilities) =>
        set((s) => {
          ...
          return {
            bots: next,
            // A catalogue is a live availability snapshot, not a deletion
            // receipt. Keep histories for profiles that are temporarily absent
            // so a transient empty response cannot be persisted as data loss.
            messages: s.messages,
      ...
      upsertEventMessage: ({ profile, conversation, messageId, role, text, createdAt, historical, keepRole, streaming, attachments }) => {
        ...
        if (resetAt && typeof createdAt === "number" && createdAt <= resetAt) return;
        void conversation;
      ...
      upsertSessionHistoryMessages: (profile, conversation, input) => {
        ...
        const resetAt = get().historyResetAt;
        const rows = resetAt ? input.filter((row) => typeof row.createdAt !== "number" || row.createdAt > resetAt) : input;
        if (!rows.length) return;
        void conversation;
        set((s) => ({ messages: rows.reduce((messages, row) => mergeEventMessage(messages, bot.id, row), s.messages).sort((a, b) => a.createdAt - b.createdAt) }));
      },
    ...
    }),
    {
      ...
      partialize: (s) => ({
        ...
        messages: settleOrphanPending(liveBotMessages(s.messages.map(sanitizeMessage))),
        messageOrigin: s.messageOrigin,
        historyResetAt: s.historyResetAt,
